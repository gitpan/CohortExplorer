package CohortExplorer::Application::Opal::Version::1::Datasource;
...
sub variable_structure {
	...
	if ( $username ne "administrator" ) {
		...
		$acl{-or} = [
			"vt.name" => {
				-in => \"SELECT DISTINCT IF(permission IN ('TABLE_ALL', 'TABLE_VALUES', 'ADMINISTRATE', 'VIEW_ALL'), REPLACE(node, '/datasource/$datasource_name/table/', ''), '')  FROM subject_acl WHERE principal = '$username' AND permission IN ('TABLE_ALL', 'TABLE_VALUES', 'VIEW_ALL', 'ADMINISTRATE')"
			},
			"CONCAT(vt.name,'.',var.name)" => {
				-in => \"SELECT DISTINCT IF(permission = 'VARIABLE_READ', REPLACE (node, '/datasource/$datasource_name/table/', ''), '') FROM subject_acl WHERE principal = '$username' AND permission = 'VARIABLE_READ'"
			}
		];
	}
...
CohortExplorer::Application::Opal::Version::1::Datasource - CohortExplorer class to initialise datasource stored under L<Opal v1.0 (OBiBa)|http://obiba.org/node/63> SQL framework
